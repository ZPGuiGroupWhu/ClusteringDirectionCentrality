...
import com.yolo.CDC.java.knnsearch.index.{IndexBase, IndexKDTree, IndexKDTreeSingle, IndexKMeans, SearchParamsBase}
...
object HCDC_parallel_v2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //设置近邻搜索参数: KDTreeSingle/Kmeans/RandomizedKDTrees
    PointRDDIndexType = "KDTreeSingle"
  ...
  def createSearchParams(): SearchParamsBase = {
    PointRDDIndexType match {
      case "Kmeans" => {
      ...
      case "KDTreeSingle" => {
        val searchParams: SearchParamsBase = new IndexKDTreeSingle.SearchParams()
        searchParams.eps = 0.0f
        searchParams.maxNeighbors = k
        searchParams
      }
      case "RandomizedKDTrees" => {
        val searchParams: SearchParamsBase = new IndexKDTree.SearchParams()
        searchParams.eps = 0.0f
        searchParams.maxNeighbors = k
        searchParams.checks = 128
        searchParams
      }
    }
  }
  ...
  def createBroadcastIndex(data: Array[Array[Double]]): Broadcast[IndexBase] = {
    ...
    PointRDDIndexType match {
      ...
      case "KDTreeSingle" => {
        val buildParams: IndexKDTreeSingle.BuildParams = new IndexKDTreeSingle.BuildParams()
        val indexKDTreeSingle: IndexBase = new IndexKDTreeSingle(metric, data, buildParams)
        indexKDTreeSingle.buildIndex()
        sc.broadcast(indexKDTreeSingle)
      }
      case "RandomizedKDTrees" => {
        val buildParams: IndexKDTree.BuildParams = new IndexKDTree.BuildParams(4)
        val indexKDTree: IndexBase = new IndexKDTree(metric, data, buildParams)
        indexKDTree.buildIndex()
        sc.broadcast(indexKDTree)
      }
    }
  ...
  def paintClusterResult(list: List[CDCPoint], outputPath: String) = {
    ...
    val chart: JFreeChart = ChartFactory.createScatterPlot("HCDC(k=" + k + ",tDCM=" + new DecimalFormat("0.00").format(dcmThreshold) + ",ratio=" + dcmRatio + ")", "X", "Y", xydatasetbc.value, PlotOrientation.VERTICAL, true, false, false)
  ...
  def paintClusterAndPartitionResult(list: List[CDCPoint], grids: List[Envelope], outputPath: String) = {
    ...
    val chart: JFreeChart = ChartFactory.createScatterPlot("HCDC(k=" + k + ",partition=" + PointRDDGridType + ",num=" + PointRDDNumPartitions + ")", "X", "Y", xydatasetbc.value, PlotOrientation.VERTICAL, true, false, false)
    chart.setBackgroundPaint(Color.white)
    chart.setBorderPaint(Color.GREEN)
    chart.setBorderStroke(new BasicStroke(1.5f))
    chart.removeLegend()
      ...
      for (j <- List(4, 8, 16, 32)) {
        ...
        for (m <- Range(10, 51, 5)) {
          ...
          for (n <- Range(5, 31, 5)) {
            ...
            val searchParams: SearchParamsBase = createSearchParams()
            searchParams.maxNeighbors = k
            txtDataInputLocation = resourceFolder + "/SyntheticDatasets/" + dataFileName + ".txt"
            clusterResultOutputLocation = resourceFolder + "/ResultData/clusterResult/HCDC/" + dataFileName + "/" + PointRDDGridType + "_" + PointRDDIndexType + "_" + PointRDDNumPartitions + "_" + k + "_" + dcmRatio + ".png"
            clusterAndPartitionResultOutputLocation = resourceFolder + "/ResultData/cpResult/HCDC/" + dataFileName + "/" + PointRDDGridType + "_" + PointRDDIndexType + "_" + PointRDDNumPartitions + "_" + k + "_" + dcmRatio + ".png"
