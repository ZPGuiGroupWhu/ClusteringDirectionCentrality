...
object KCDC_parallel_v2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //设置分区参数
    PointRDDGridType = GridType.None
  ...
  def paintClusterResult(list: List[CDCPoint], outputPath: String) = {
    ...
    val chart: JFreeChart = ChartFactory.createScatterPlot("KCDC(k="+k+",tDCM="+new DecimalFormat("0.00").format(dcmThreshold)+",ratio="+dcmRatio+")", "X", "Y", xydatasetbc.value, PlotOrientation.VERTICAL, true, false, false)
  ...
  def paintClusterAndPartitionResult(list: List[CDCPoint], grids: List[Envelope], outputPath: String) = {
    ...
    val chart: JFreeChart = ChartFactory.createScatterPlot("KCDC(k="+k+",partition="+PointRDDGridType+",num="+PointRDDNumPartitions+")", "X", "Y", xydatasetbc.value, PlotOrientation.VERTICAL, true, false, false)
    chart.setBackgroundPaint(Color.white)
    chart.setBorderPaint(Color.GREEN)
    chart.setBorderStroke(new BasicStroke(1.5f))
    chart.removeLegend()
      ...
      for (j <- List(4,8,16,32)) {
        ...
        for (m <- Range(10, 51, 5)) {
          ...
          for (n <- Range(5, 31, 5)) {
            ...
            clusterResultOutputLocation = resourceFolder + "/ResultData/clusterResult/KCDC/" + dataFileName + "/" + PointRDDGridType + "_" + PointRDDIndexType + "_" + PointRDDNumPartitions + "_" + k + "_" + dcmRatio + ".png"
            clusterAndPartitionResultOutputLocation = resourceFolder + "/ResultData/cpResult/KCDC/" + dataFileName + "/" + PointRDDGridType + "_" + PointRDDIndexType + "_" + PointRDDNumPartitions + "_" + k + "_" + dcmRatio + ".png"
