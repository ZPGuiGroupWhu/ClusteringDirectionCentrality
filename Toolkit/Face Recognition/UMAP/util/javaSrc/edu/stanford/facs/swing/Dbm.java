...
public class Dbm {
	public static ArrayList<String> GetGateNames(
			final boolean isX1stName,
			final double []means, 
			final String xName, 
			final String yName) {
		final ArrayList<String> out=new ArrayList<>();
		final String x1=GetExpressionTag(means[0]), 
				x2=GetExpressionTag(means[2]),
				y1=GetExpressionTag(means[1]),
				y2=GetExpressionTag(means[3]);
		final boolean useX=!x1.equals(x2), useY=!y1.equals(y2);
		final boolean theSame=!useX && !useY;
		final boolean bothDiffer=useX && useY;
		if (theSame) {
			final String name;
			if (isX1stName) {
				name=xName+x1+" "+yName+y1;
			}else {
				name=yName+y1+" " + xName+x1;
			}
			out.add(name+"#1");
			out.add(name+"#2");
		} else if (bothDiffer) {
			if (isX1stName) {
				out.add(xName+x1+" "+yName+y1);
				out.add(xName+x2+" "+yName+y2);
			} else {
				out.add(yName+y1+" "+xName+x1);
				out.add(yName+y2+" "+xName+x2);
	...
	public static String GetExpressionTag(final double expression) {
		if (expression>=.63)
			return "++";
		if (expression>=.45)
			return "+";
