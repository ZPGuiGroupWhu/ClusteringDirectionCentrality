...
CDC <- function(dat_mat, k, ratio, embeding_method="UMAP", k_UMAP=NULL, npc=NULL, norm=NULL){
  ...
  if(is.null(npc)){
    npc <- max(2, min(dim, as.integer(log2(k))))
  }
  ...
  assertthat::assert_that(embeding_method %in% c("PCA", "UMAP", "None"))
...
ACC <- function (ref,res){
  n = length(ref)
  p = sort(unique(ref))
  c = sort(unique(res))
  P_size = length(p)
  C_size = length(c)
  P_id <- (array(1, dim=c(P_size,1))%*%array(ref, dim=c(1,n)) == array(p, dim=c(P_size,1))%*%array(1, dim=c(1,n)))
  C_id <- (array(1, dim=c(C_size,1))%*%array(res, dim=c(1,n)) == array(c, dim=c(C_size,1))%*%array(1, dim=c(1,n)))
  CP <- C_id %*% t(P_id)
  pair_id  <- HungarianSolver(-CP+max(CP))$pairs
  cost <- sum(CP[pair_id])
  acc <- cost/n
  return(acc)
}
