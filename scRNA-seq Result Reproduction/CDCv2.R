...
CDCv2 <- function(dat_mat, k, ratio, embeding_method=NULL, k_UMAP=NULL, npc=NULL, norm=NULL){
...
con_int_pts <- function(X, reach_dis, num){
  ...
  clust_id <- 1
  for(i in 1:num){
    if(reach_dis[i,2]==1&int_clust[i]==0){
      int_clust[i] <- clust_id
      for(j in 1:num){
        if(reach_dis[j,2]==1&sqrt((X[i,]-X[j,])%*%(X[i,]-X[j,]))<=reach_dis[i,1]+reach_dis[j,1]){
          if(int_clust[j]==0){
            int_clust[j] <- clust_id
          }else{
            temp_clust_id <- int_clust[j]
