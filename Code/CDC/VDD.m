function cluster = VDD(k_num, T, X)
[n,m] = size(X);
...
for i=1:n
    vec = zeros(k_num,m);
    for j=1:k_num
        orig_vec = X(get_knn(i,j),:)-X(i,:);
        vec(j,:) = orig_vec/sqrt(orig_vec*orig_vec');
    end
    y_dcm = zeros(m,1);
    for j=1:m
        sort_x = sort(vec(:,j));
        dis = zeros(k_num-1,1);
        for k=1:k_num-1
            dis(k) = sort_x(k+1)-sort_x(k);
        end
        y_dcm(j)=(k_num-1)*(dis-2/(k_num-1))'*(dis-2/(k_num-1))/(4*k_num-8);
    end
    DCM(i) = max(y_dcm);
end
